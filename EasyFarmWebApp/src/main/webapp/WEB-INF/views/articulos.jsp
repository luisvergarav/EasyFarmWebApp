		...
		<div class="row">
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<i class="fa fa-bell fa-fw"></i> Articulos disponibles
							<!-- Button trigger modal -->
							
									
									<a data-toggle="modal" class="btn btn-primary btn-sm  pull-right" href="addArticulo" data-target="#myModal">Agregar</a>
					</div>
					<!-- /.panel-heading -->
					<div class="panel-body">
						<div class="list-group">
							<table id="tabla1" cellpadding='3' cellspacing='2'
								...
								<c:if test="${listadoArticulos.size() <= 0}">
									<tr>
										<td colspan="5">0 Articulos</td>
									</tr>
								</c:if>
								<c:forEach items="${listadoArticulos}" var="articulo">
									<tr>

										<td>
											<div
												<c:choose>
                                        <c:when test="${articulo.nombreArticulo == 'HOCKEY'}">class="deportes_sprite2 hockey"
                                        </c:when>                                     
                                        <c:when test="${articulo.nombreArticulo == 'TENIS'}">class="deportes_sprite2 tenis"
                                        </c:when> 
                                        <c:otherwise>
										<class="deportes_sprite2 tenis" 
											...
											<div>
												<c:out value="${articulo.nombreArticulo}" />
											</div>
										</td>
										...
										<td>
											<div>
												<c:out
													value="$${articulo.precio} - ${articulo.tipoArticulo.descripcion}" />
											</div>
											<div>
												<c:out value="Unidad Medida ${articulo.unidadMedida}" />
											</div>
										</td>
										...
										<td><a onclick="return confirm('esta seguro?')"
											href="<c:url value="/inscripciones/agregarSeccionInscripcion.do?idTaller=${articulo.id.idArticulo}" />">Secciones</a></td>
									</tr>
								</c:forEach>
